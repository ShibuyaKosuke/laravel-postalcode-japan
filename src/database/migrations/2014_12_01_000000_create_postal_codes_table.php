...
class CreatePostalCodesTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('postal_codes', function (Blueprint $table) {
            ...
            $table->boolean('flag_doubleCode')->comment('一町域が二以上の郵便番号で表される場合の表示');
            $table->boolean('flag_banchi')->comment('小字毎に番地が起番されている町域の表示');
            $table->boolean('flag_chome')->comment('丁目を有する町域の場合の表示');
            $table->boolean('flag_double_area')->comment('一つの郵便番号で二以上の町域を表す場合の表示');
            $table->boolean('flag_update')->comment('更新の表示');
            $table->boolean('flag_update_reason')->comment('変更理由');
